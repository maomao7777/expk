...
int ctrl_iface_init(clockstat *p)
{
    struct sockaddr_un *addr = &p->ctrl_addr;
    int s = -1;
    char fname[128] = {0};
    ...
    sprintf(fname, "%s/%s", CTRL_INTERFACE, CTRL_INTERFACE_IF);
...
void read_event_recv(int socket_fd, void *eloop_ctx, void *timeout_ctx)
{
    ...
    len = (socket_fd==p->ctrlfd)?
    recvfrom(socket_fd, string, 127, 0, (struct sockaddr *) &from, &fromlen):
    read(socket_fd, string, 127);
    printf("recv[%d]\n",len);
    //usleep(10);
    if(len>0)
    {
        ...
        {
            sprintf(rbuf,"%s","fuck_ur_mother");
        }
        else
        {
            sprintf(rbuf,"%s","what_u_fucking_tell_me");
        }
        ret=(socket_fd==p->ctrlfd)?
        sendto(socket_fd, rbuf, strlen(rbuf), 0, (struct sockaddr *) &from, fromlen):
        write(socket_fd, rbuf, strlen(rbuf));
        if(ret < 0)
            printf("err:%s response[%d]\n",__func__,ret);
    }
    else
    {
...
#endif
/*-----------------------------------------------------------------------------------------------------------------------*/
void ctrl_event_recv(int socket_fd, void *eloop_ctx, void *timeout_ctx)
{
    printf("Receive ctrl conn!\n");
    read_event_recv(socket_fd,eloop_ctx,NULL);
}
/*-----------------------------------------------------------------------------------------------------------------------*/
void ser_event_recv(int socket_fd, void *eloop_ctx, void *timeout_ctx)
{
    ...
    p->cnet = accept(socket_fd, (struct sockaddr *) &cli_addr, &clilen);
    printf("acept new conn\n");
...
/* listen to the socket */
    ...
    eloop_register_read_sock(p->ctrlfd, ctrl_event_recv,p, NULL);
    eloop_register_timeout(1,0,chk_clockstat,p,NULL);
    eloop_run();
    goto out;
